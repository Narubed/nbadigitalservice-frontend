...
function mainImage(props) {
  ...
  const [imgSrcMain, setImgSrcMain] = useState(null);
  const [fileMain, setfileMain] = useState([]);
  const [isWidth, setWidth] = useState('');

  useEffect(() => {
    if (isValues) {
      setWidth(isValues.mainImage_width);
    }
  }, [isValues]);
  ...
  const onChangeMain = (file) => {
    ...
    if (files && files.length !== 0) {
      setfileMain(files[0]);
      reader.onload = () => setImgSrcMain(reader.result);
  ...
  return (
    <div>
      ...
      {imgSrcMain ? (
        <img src={imgSrcMain} alt={imgSrcMain} />
      ) : (
        <img
          src={`${process.env.REACT_APP_WEB_SERVICE}/static/first-page/${isValues.mainImage}`}
          alt={`${process.env.REACT_APP_WEB_SERVICE}/static/first-page/${isValues.mainImage}`}
        />
      )}
